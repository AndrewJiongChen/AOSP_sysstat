...
#pragma GCC diagnostic ignored "-Wpointer-arith"
			memset(a->buf[j] + a->msize * a->nr_allocated * a->nr2, 0, (size_t) a->msize * (size_t) (nr_realloc - a->nr_allocated) * (size_t) a->nr2);
		       ...
		       int ignore, int *endian_mismatch, int *arch_64)
{
	...
	*arch_64 = (file_hdr->sa_sizeof_long == SIZEOF_LONG_64BIT);
	...
	/* Read activity list */
	j = 0;
	for (i = 0; i < (int)file_hdr->sa_act_nr; i++, fal++) {
		...
		*/
		...
		if (MAP_SIZE(fal->types_nr) > (unsigned int)fal->size) {
#ifdef DEBUG
	...
	for (i = 0; i < NR_ACT; i++) {
		...
		fal = *file_actlst;
		for (j = 0; j < (int)file_hdr->sa_act_nr; j++, fal++) {
			if (act[i]->id == fal->id)
				break;
		}
		if (j == (int)file_hdr->sa_act_nr) {
			/* No: Unselect it */
			act[i]->options &= ~AO_SELECTED;
		}
	}
