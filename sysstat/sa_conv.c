			   ...
			   struct old_file_activity **ofile_actlst)
{
	...
	for (i = 0; i < (int)file_hdr->sa_act_nr; i++, ofal++) {
...
void upgrade_stats_cpu(struct activity *act[], int p, int st_size)
{
	...
	for (i = 0; i < act[p]->nr_ini; i++) {
		...
		if (st_size >= (int)STATS_CPU_8A_SIZE) {
			  ...
			  int endian_mismatch, int arch_64)
{
	...
	if (st_size >= (int)STATS_MEMORY_8A_SIZE) {
			     ...
			     int endian_mismatch, int arch_64)
{
	...
	for (i = 0; i < (int)file_hdr->sa_act_nr; i++, ofal++, fal++) {
